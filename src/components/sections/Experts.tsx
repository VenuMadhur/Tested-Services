...
function PanelStat({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div ref={ref}>
      ...
      <div className="mt-1 text-xs font-medium uppercase tracking-wider text-emerald-100/80">{label}</div>
    </div>
  );
}
...
export default function Experts() {
  return (
    <section id="experts" className="relative bg-brand-50 py-24 sm:py-32">
      <Container>
        <SectionHeading
        ...
        <div className="mt-16 grid gap-10 lg:grid-cols-[0.85fr,1.15fr] lg:items-center lg:gap-14">
          <motion.div
            ...
            className="relative mx-auto flex w-full max-w-sm flex-col overflow-hidden rounded-3xl bg-gradient-to-br from-brand-900 via-brand-800 to-emerald-700 p-8 text-white shadow-card-hover lg:max-w-none"
          >
            <div className="pointer-events-none absolute -right-14 -top-14 h-48 w-48 rounded-full bg-white/10 blur-3xl" />
            <ShieldCheck className="h-9 w-9 text-emerald-200" strokeWidth={1.5} />
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {roles.map((role, i) => (
              ...
              >
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-brand-700 to-emerald-500 text-white shadow-card transition-transform duration-300 group-hover:scale-105">
